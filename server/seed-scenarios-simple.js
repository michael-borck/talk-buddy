// Simple scenario seeder (no auth required)
// 
// BEFORE RUNNING:
// 1. Go to PocketBase Admin UI
// 2. Edit scenarios collection
// 3. Change "Create rule" to empty (allow all)
// 4. Run this script
// 5. Change "Create rule" back to: @request.auth.id != ""
...
async function seedScenarios() {
  ...
  console.log('🌱 TalkBuddy Scenario Seeder (No Auth)\n');
  console.log('⚠️  IMPORTANT: Make sure you\'ve temporarily removed the create rule!\n');
  ...
  if (success > 0) {
    console.log('\n⚠️  REMINDER: Go back to PocketBase admin and restore the create rule!');
    console.log('   Set "Create rule" back to: @request.auth.id != ""');
  }
}
